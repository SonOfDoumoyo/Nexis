...
export function CreateButton({text, type, arrow,onClick = ()=>null}:buttonProps){
    const isMobile = useIsMobile();
    return (
        <button className={`${type == 1 ? 'bg-linear-to-r shadow-lg shadow-bright-purple from-deep-purple to-bright-purple':'backdrop-blur-[1px] border border-warm-gold'} ${isMobile == true ? 'py-2':'py-3'} px-6 text-[clamp(0.5rem,0.9vw,1.45rem)] rounded-full outline-0 cursor-pointer flex flex-row items-center gap-3 h-full text-center justify-between`} onClick={()=>onClick()}>{text}{arrow == true && (<FiArrowRight className="text-white"/>)}</button>
    )
}

export default function HeroSection(){
    const [showNav, setShowNav ] = useState<string>('hidden');
    const {openModal,setOpenModal} = useFormContext().modalContext
    const isMobile = useIsMobile()
    return (
        <section id="home" className={`${isMobile == true ? 'gap-50':'gap-40'}  flex flex-col w-full`}>
            {openModal ? <FormSection />:null}
            <div className="flex flex-row justify-between relative items-center w-full">
                <img src={NexisLogo} alt="Nexis logo" className="w-20" onClick={()=>{setOpenModal(true);console.log('set')}}/>
                <nav className={`${isMobile == true ? `${showNav} absolute right-0 items-start flex flex-col bg-deep-purple/30 backdrop-blur-[3px] z-20 top-0 w-[50%] rounded-md`:'flex flex-row gap-6'} list-none`}>
                ...
                <div className="flex flex-row gap-2 items-center">
                    <CreateButton text="Let's Build" type={2} arrow={false} onClick={()=>{setOpenModal(true);console.log('set')}}/>
                    {isMobile == true && (<span className={`${isMobile ? 'p-2':'p-3'} rounded-full bg-transparent border border-bright-gold justify-center items-center`} onClick={() => {setShowNav('block')}}><Menu size={11} strokeWidth={1.5}/></span>)}
                </div>
            </div>
            <div className="flex flex-col gap-5">
                ...
                <div className={`${isMobile == true ? 'flex-row':'flex-row'} h-full flex gap-3`}>
                    <CreateButton text="Explore Our services" type={1} arrow={true} onClick={()=>{
                        document.getElementById("service")?.scrollIntoView({
                            behavior: "smooth",
                            block: "start",
                        })
                    }}/>
                    <CreateButton text="Build Your Future" type={2} arrow={false} onClick={()=>{setOpenModal(true)}}/>
                </div>
